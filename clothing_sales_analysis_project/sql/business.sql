...
SELECT gender, 
       ...
       COUNT(*) * 100.0 / (SELECT COUNT(*) FROM sales) AS customer_percentage,
       SUM(purchase_amount) * 100.0 / (SELECT SUM(purchase_amount) FROM sales) AS sales_percentage
FROM sales
GROUP BY gender
...
SELECT payment_method,
       ...
       COUNT(*) * 100.0 / (SELECT COUNT(*) FROM sales) AS usage_percentage
FROM sales
GROUP BY payment_method
ORDER BY total_sales DESC;

-- Sales by Size & Color (detailed breakdown)
SELECT size, color, 
       COUNT(*) AS orders,
       SUM(purchase_amount) AS total_sales,
       AVG(purchase_amount) AS avg_order_value
...
-- Why Repeat Purchase Rate is 0.0% - Analysis
-- Each customer_id appears exactly once in the dataset
SELECT 'Unique Customers' AS metric, COUNT(DISTINCT customer_id) AS value FROM sales
UNION ALL
SELECT 'Total Transactions' AS metric, COUNT(*) AS value FROM sales
UNION ALL
SELECT 'Customers with Multiple Orders' AS metric, 
       SUM(CASE WHEN order_count > 1 THEN 1 ELSE 0 END) AS value
FROM (
